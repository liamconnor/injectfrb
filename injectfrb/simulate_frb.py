...
def plot_four_frbs():
    ...
    data_apertif, p = gen_simulated_frb(NFREQ=NFREQ, NTIME=NTIME, sim=True, fluence=1., 
    ...
    plt.text(NTIME*dt*.75, freq[NFREQ//2], 'Breakthrough Listen\nNo noise \nBlue spectrum\
    ...
    data_chime, p = gen_simulated_frb(NFREQ=NFREQ, NTIME=NTIME, sim=True, fluence=1., 
    ...
    plt.text(NTIME*dt*.75, freq[NFREQ//2], 'CHIME\nNo noise \nScattered\
                                      \nDM=%d' % (dm), alpha=1.0,
    ...
    data_noise_breakthrough, p = gen_simulated_frb(NFREQ=NFREQ, NTIME=NTIME, 
    ...
    data_noise_dm2000, p = gen_simulated_frb(NFREQ=NFREQ, NTIME=NTIME, 
    ...
    plt.xlabel('Time [sec]')
    plt.text(NTIME*dt*.75, freq[NFREQ//2], 'Parkes\nGaussian noise\nDM=%d' % dm, 
             alpha=1.0, bbox=dict(facecolor='white', alpha=1.0))

    plt.savefig('./test_fig_4FRBs.pdf')
    plt.show()
